...
/**
 * Select a third serial port on the board to use for communication with the
 * host. Currently only supported for AVR, DUE, LPC1768/9 and STM32/STM32F1
 * :[-1, 0, 1, 2, 3, 4, 5, 6, 7]
 */
 ...
 *   PRUSA_MMU2S          : Průša MMU2S (Requires MK3S extruder with motion
 * sensor, EXTRUDERS = 5) EXTENDABLE_EMU_MMU2  : MMU with configurable number of
 * filaments (ERCF, SMuFF or similar with Průša MMU2 compatible firmware)
 *   EXTENDABLE_EMU_MMU2S : MMUS with configurable number of filaments (ERCF,
...
// #define GRADIENT_VTOOL       // Add M166 T to use a V-tool index as a
// Gradient alias
#endif
#endif
...
#define PID_FUNCTIONAL_RANGE                                                   \
  10 // If the temperature difference between the target temperature and the
     // actual temperature is more than PID_FUNCTIONAL_RANGE then the PID will
     // be shut off and the heater will be set to min/max.
#endif
...
// probes (to improve accuracy) #define WAIT_FOR_HOTEND         // Wait for
// hotend to heat back up between probes (to improve accuracy & prevent cold
// extrude)
#endif
...
// #define UBL_MESH_WIZARD         // Run several commands in a row to get a
// complete mesh
...
// controller #define ZONESTAR_12864OLED          // 1.3" OLED with SH1106
// controller (default) #define ZONESTAR_12864OLED_SSD1306  // 0.96" OLED with
// SSD1306 controller
...
// background LED #define NEOPIXEL_BKGD_INDEX_LAST   5              // Index of
// the last background LED #define NEOPIXEL_BKGD_COLOR { 255, 255, 255, 0 }  //
// R, G, B, W #define NEOPIXEL_BKGD_ALWAYS_ON                   // Keep the
// backlight on when other NeoPixels are off
#endif
